/***************************************************
Author: Tae Hyeong Moon & Ricky Morri
Course: CSC-213-01
Professor: Charlie Curtsinger
Due Date: November 9, 2022
Lab: GPU Sudoku Solver
Help Received: Charlie Curtsinger, Jeremy Trachtenberg, Mingi Lee and Benjamin Stern

Citation:
1.2022 Fall CSC-213-01 GPU demo
*****************************************************/
#include <stdbool.h>
#include <stdint.h>
#include <stdio.h>
...
/**
 * GPU function of solve_boards that takes one board and one cell in that board
 * and checks for all the constraints that it can remove for that iteration of
 * the board. This is repeated until either the board is solved or we cannot
 * change anything else in the board, declaring it unsolvable for the given
 * algorithm.
 *
 * \param boards      An array of boards that should be solved.
 */
__global__ void solve_boards_gpu(board_t* boards) {
  /*
  Procedures for removing constraints:

  Check if it's not a specific digit
  if not specific start checking for constraints
  check box first
    -> find the place the current digit is at in the box (eg (0, 0), (1, 2)).
       and then use those coordinates to find the top left number by moving/
       calculating. then use a forloop to just traverse the box starting from
       the top left and exclude the current digit from the check.
  check row
    -> add 1 to the index and check for that digit. if it's not a specific digit
       skip to the next index. If it is a specific digit, then subtract that from
       the current index we're checking.
    -> divide the index by 9 and floor the number to get the starting index of
       that row, then add 1 to the index in a loop to check each constraint.
  check column
    -> add 9 to the index and check for that digit. if it's not a specific digit
       skip to the next index. If it is a specific digit, then subtract that from
       the current index we're checking.
    -> to find the column of numbers we want to do index % 9 to get the index of
       the digit of the first row for that column. Add 9 to that in a loop to check
       all the digits (except the current index we're on).
  */
  ...
  do {
    if (cell_to_digit(board->cells[index]) != 0) {
      break;
    }

    changed = 0;
    int box_col_move = threadIdx.x % 3;
    int box_row_move = (threadIdx.x / 9) % 3;

    /*
    Box checking:
    We find the top left index of the box and traverse through the box from there going from left
    to right for every row in the box.
    */
    for (int i = index - box_col_move - (9 * box_row_move), count = 0; count < 9; i++) {
      if (cell_to_digit(board->cells[i]) != 0 && i != index) {
        // modify cell
        if (board->cells[index] != (board->cells[index] & ~(1 << cell_to_digit(board->cells[i])))) {
          changed = 1;
        }
        board->cells[index] = board->cells[index] & ~(1 << cell_to_digit(board->cells[i]));
      }
    ...
    /* Row checking:
    Find the row number and start from the leftmost index and traverse right through the row
    */
    for (int i = (index / 9) * 9, count = 0; count < 9; i++, count++) {
      if (cell_to_digit(board->cells[i]) != 0 && i != index) {
        // modify cell
        if (board->cells[index] != (board->cells[index] & ~(1 << cell_to_digit(board->cells[i])))) {
          changed = 1;
    ...
    /*Column checking
    Find the column number and start at the topmost index and traverse down the column
    */
    for (int i = index % 9, count = 0; count < 9; i += 9, count++) {
      if (cell_to_digit(board->cells[i]) != 0 && i != index) {
        // modify cell
        if (board->cells[index] != (board->cells[index] & ~(1 << cell_to_digit(board->cells[i])))) {
          changed = 1;
...
void solve_boards(board_t* boards, size_t num_boards) {
  /*
    send the board into the kernel
  */
  ...
  if (cudaMalloc(&gpu_boards, sizeof(board_t) * num_boards) != cudaSuccess) {
    fprintf(stderr, "Failed to allocate boards array on GPU\n");
    exit(2);
  }
  ...
  solve_boards_gpu<<<blocks, threads_per_block>>>(gpu_boards);
